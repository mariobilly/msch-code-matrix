...
| Node | What it does |
|---|---|
| [Code Matrix (ASCII)](docs/NODES.md#codematrixascii) | Rebuild each input frame from a monospace glyph atlas. |
...
```bash
git clone https://github.com/mariobilly/msch-code-matrix.git
```
...
```powershell
.\python_embeded\python.exe -m pip install -r .\ComfyUI\custom_nodes\msch-code-matrix\requirements.txt
```
...
Processes IMAGE batches. It does not decode video or carry audio; use native video frame conversion or VideoHelperSuite for loading and encoding clips. Keep audio on a separate connection to the video encoder.
